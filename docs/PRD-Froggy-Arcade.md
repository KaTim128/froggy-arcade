...
- **Green is a promise, so it is cautious.** The rim moves, so the prediction walks it forward a second and a half to where it will be when the ball arrives, and a walk at a fixed step against a flight integrated at whatever the frame rate gives drifts a pixel or two over that distance. A shot that clears the mouth by less than the drift shows **amber**: the arc will not claim what it cannot keep. Green never lies; amber occasionally drops anyway, which is a pleasant surprise rather than a broken word.
- **A make is judged AT THE CROSSING.** The ball covers five or six pixels a frame and the mouth is twenty wide, so testing where it was at the end of the step that took it past the rim judged the shot on where it had got to rather than on where it went through — a ball that dropped cleanly through one edge was a miss because a frame later it was outside. The path within a step is a straight line, so the test interpolates where that line crosses the rim's height. Asserted by `tools/games.mjs`, which asks the arc for a shot it calls good and then takes it.
